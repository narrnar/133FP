...
Tyler_corp <- tm_map(Tyler_corp, remove_custom_punctuation)  
...
Doom_corp <- tm_map(Doom_corp, remove_custom_punctuation)  
...
head(topic_terms)
albums_top10 <- topic_terms %>%
  group_by(topic) %>%
  slice_max(beta, n = 6) %>%
  ungroup() %>%
  arrange(topic, -beta)
albums_top10
ggplot(albums_top10, aes(x = beta, reorder_within(term, beta, topic), fill = factor(topic))) +
  geom_col(show.legend = FALSE) +
