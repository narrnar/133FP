...
censor_words <- function(text) {
  text <- gsub("\\b(n|N)(i|I)(g|G){2}", "\\1\\2**", text, perl = TRUE)
  text <- gsub("\\b(f|F)(a|A)(g|G){2}", "\\1\\2**", text, perl = TRUE)
  return(text)
}
...
# Top bigrams across both arists
# Word Clouds

# Correlation Plot
# term_freq <- colSums(gandhi_mat_sparse)
# terms_freq_sorted <- sort(term_freq, decreasing = TRUE) head(terms_freq_sorted, 20)
# plot(gandhi_dtm_sparse, terms = findFreqTerms(gandhi_dtm_sparse, lowfreq = 20), corThreshold = 0.33)
```
# Sentiment Analysis
